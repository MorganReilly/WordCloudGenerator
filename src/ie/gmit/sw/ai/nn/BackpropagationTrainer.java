...
public class BackpropagationTrainer implements Trainator {
	...
	public void train(double[][] trainingData, double[][] desired, double alpha, int epochs) {
		...
		for (; epoch < epochs && hasError; epoch++) {
			...
			for (int index = 0; index < trainingData.length; index++) {
				...
				for (int i = 0; i < net.getInputLayer().length; i++)
					net.getInputLayer()[i] = sample[i];
				for (int i = 0; i < net.getOutputLayer().length; i++)
					net.getOutputLayer()[i] = expected[i];

				net.feedForward();
				backpropagate(expected, alpha);
			}

			for (int i = 0; i < err_out.length; i++)
				sumOfSquaresError += Math.pow(err_out[i], 2);

			// if (epoch % 1000 == 0) System.out.println(epoch + "," + sumOfSquaresError);
			if (Math.abs(sumOfSquaresError) <= errTolerance)
				hasError = false; // Bail out of training
		}
		System.out.println(
				"[INFO] Training completed in " + ((System.currentTimeMillis() - startTime) / 1000) + " seconds.");
	...
	private void backpropagate(double[] expected, double alpha) { //
		// Looping over each node in hidden layer
		// Each node in hidden is wired to nodes in output
		...
		for (int out = 0; out < net.getOutputLayer().length; out++) {
			...
			err_out[out] = net.getActivator().derivative(net.getOutputLayer()[out])
					* (expected[out] - net.getOutputLayer()[out]);
		}
